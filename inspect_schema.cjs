...
async function inspectSchema() {
    ...
    `);
        ...
        const templatesCols = await query(`
      SELECT column_name, data_type 
      FROM information_schema.columns 
      WHERE table_name = 'material_templates' AND table_schema = 'public'
    `);
        console.log('\n--- material_templates columns ---');
        templatesCols.rows.forEach(r => console.log(`${r.column_name}: ${r.data_type}`));
